...
export class AuthManagementComponent {
  ...
  userStatusFilter = signal<'all' | 'active' | 'pending'>('all');

  totalUserCount = computed(() => this.companyUsers().length + this.pendingInvitations().length);
  activeUserCount = computed(() => this.companyUsers().length);
  pendingUserCount = computed(() => this.pendingInvitations().length);

  filteredCompanyUsers = computed(() => {
    ...
    return this.companyUsers().filter(u => {
      ...
      return (u.fullName || '').toLowerCase().includes(term)
        ...
        || (u.roleInCompany || '').toLowerCase().includes(term);
    });
  });

  filteredPendingInvitations = computed(() => {
    const term = this.userSearchTerm().trim().toLowerCase();
    const status = this.userStatusFilter();
    if (status === 'active') return [];
    return this.pendingInvitations().filter(inv => {
      if (!term) return true;
      return (inv.email || '').toLowerCase().includes(term)
        || (inv.roleInCompany || '').toLowerCase().includes(term);
    });
  });

  filteredTotal = computed(() => this.filteredCompanyUsers().length + this.filteredPendingInvitations().length);
...
selectedDeleteUser = signal<any | null>(null);
selectedDeleteInvitation = signal<any | null>(null);
  // ============================================
  // LIFECYCLE HOOKS
  // ============================================
  Math = Math;
...
getInvitationUserName(email: string): string {
  const existingUser = this.companyUsers().find(
...
onSearchUser() {
  ...
  sendInvitation() {
    ...
    if (!email || !role) {
       this.showToast('Please select a user and role', 'error');
      return;
    }
    ...
    error: (err) => {
      this.showToast(err.error?.message || 'Failed to send invitation', 'error');
    }
    });
  }
  ...
  updateUserRole(userParam: CompanyUserDetailDto | null, newRole: string | null): void {
    ...
    this.svc.updateUserRole(user.userId, newRole).subscribe({
      next: (res: any) => {
        ...
        this.loadCompanyUsers();
        this.showToast(res?.message || 'Failed to update role', 'error');
      }
      },
      error: (err) => {
...
openConfirm(title: string, message: string, action: () => void) {
  ...
  this.confirmAction.set(action); // fixed
...
removeUser(userId: number) {
  this.svc.removeUserFromCompany(userId).subscribe({
    next: (res: any) => {
      if (res?.success) {
        this.loadCompanyUsers();
        this.showToast('User removed successfully');
      } else {
        this.showToast(res?.message || 'Failed to remove user', 'error');
      }
    },
    error: (err) => {
      this.showToast(err.error?.message || 'Failed to remove user', 'error');
    }
  });
}
  ...
  this.openConfirm(
    ...
    () => {
      this.svc.cancelInvitation(invitationId).subscribe({
        next: (res: any) => {
          this.loadPendingInvitations();
          this.showToast(res?.message || 'Invitation cancelled');
        },
        error: (err) => {
          this.showToast(err.error?.message || 'Failed to cancel invitation', 'error');
        }
      });
    }
  );
      ...
      this.svc.approveJoinRequest(requestId).subscribe({
        next: (res: any) => {
          if (res?.success) {
            ...
            this.showToast(res?.message || 'Failed to approve request', 'error');
          }
        },
        error: (err) => {
          this.showToast(err.error?.message || 'Failed to approve request', 'error');
        }
      });
    }
  );
